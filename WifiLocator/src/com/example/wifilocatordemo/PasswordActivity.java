...
public class PasswordActivity extends Activity {
	...
	protected void onCreate(final Bundle bundle) {
		...
		etHintText = (EditText) findViewById(R.id.etHintText);// Enter password
																// here
		etChange = (EditText) findViewById(R.id.etChange);// Enter new password
															// here to change
		...
		/*
		 * Default state of the field for changing password is false, user
		 * cannot change password before enter the right password
		 */
		...
		btChange.setOnClickListener(new OnClickListener() {
			...
			public void onClick(final View view) {
				...
				if (test.compareTo(pass) == 0) {
					...
					/*
					 * Set the state of field to enter password is false enable
					 * the field to change password
					 */
	...
	// Set activity when click button "OK" after type the password to continue
	// this app
	public void onClickOKPass(final View view) {
		...
		/*
		 * If the typed password matches the right password start new activity
		 * has name WifiActivity
		 */
		if (test.compareTo(pass) == 0) {
			etHintText.setText("");
		...
		/*
		 * If the password is wrong, create a small window with the
		 * announcement. Reset the field to enter password
		 */
		else {
			Toast.makeText(getApplicationContext(),
					"Incorrect ! Try retype password", Toast.LENGTH_SHORT)
					.show();
			etHintText.setText("");
		}
	}
	...
	/*
	 * Set the activity when user change password, reset the value of the
	 * password
	 */
	public void onClickOKChange(final View view) {
		...
		prefsEditor.putString("editTextPref", etChange.getText().toString());
		prefsEditor.commit();
